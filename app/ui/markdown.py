"""Markdown to HTML rendering for the documentation window.

Kept free of PySide6 so the rendering can be tested on a headless machine; the
widget that displays the result lives in
:mod:`app.ui.widgets.markdown_view`. Code blocks are highlighted with Pygments and
the stylesheet follows the application theme, so the viewer matches the rest of the
interface in both light and dark mode.
"""
...
# Failure messages, matched to the danger colour of the application themes.
